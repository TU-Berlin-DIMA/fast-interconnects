...
extern "C" int numa_copy(int num_threads, size_t size, uint16_t src, uint16_t dst, uint64_t *nanos)
{
...
#pragma omp parallel
    {
        ...
        ::memcpy(int_dst + tid * size_per_thread, int_src + tid * size_per_thread, size_per_thread);
    }
...
int main() {
    ...
    std::cout
        << (size / 1024ull / 1024ull / 1024ull) / (((double)avg) / 1.e9)
        << std::endl;
